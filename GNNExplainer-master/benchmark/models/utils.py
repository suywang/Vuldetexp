...
class DGLdata():
    ...
    def type_conversion(x, edge_index, edge_attr):
        ...
        edge_attr_list = edge_attr.numpy().tolist()
        for i in range(edge_index_list):
            graph.add_edge(edge_index_list[i][0], edge_index_list[i][1], data={'etype': torch.LongTensor(edge_attr_list[i][0])})
